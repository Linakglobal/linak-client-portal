...
describe('Health Check Tests', () => {
  test('GET /api/health should return healthy status', async () => {
    ...
    assert.ok(typeof data.memory.used === 'number', 'Memory used should be a number');
  });
  ...
  test('HEAD /api/health should return 200', async () => {
    ...
    assert.strictEqual(response.status, 200, 'Health HEAD request should return 200');
  });
});
...
describe('Core Page Tests', () => {
  ...
  test('404 page should work for non-existent routes', async () => {
    const response = await fetchWithTimeout(`${BASE_URL}/non-existent-route-${Date.now()}`);
...
describe('Environment Tests', () => {
  test('Required environment variables should be configured', () => {
    ...
    assert.ok(process.version.startsWith('v18.') || process.version.startsWith('v20.'), 
