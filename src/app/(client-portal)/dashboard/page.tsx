...
import { Upload, FileText, User, TrendingUp, Clock, CheckCircle, AlertCircle, BarChart3, Users, Globe, Shield } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const getStatusBadgeClass = (status: string) => {
    ...
    if (status === "pending") return "bg-gray-800/10 text-gray-300";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {dashboardStats.map((stat) => (
              <div key={stat.title} className="streamit-metric-card group">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center mt-2">
                      <span className={`text-sm ${getChangeTextColor(stat.change)}`}>
                        {stat.change}
                      </span>
                    ...
                    {recentActivities.map((activity) => (
                      ...
                      >
                        <div className="flex items-center space-x-4">
                          <div className={`w-2 h-2 rounded-full ${getStatusDotClass(activity.status)}`} />
                          <div>
